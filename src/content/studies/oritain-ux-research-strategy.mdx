...
summary:
  role: "UX Lead"
  problem: "Fashion clients struggled to extract value from Oritain’s cotton traceability data due to disconnected tools, manual workflows, and misaligned timelines."
...
The core issue: **a deterministic interface showing aggregate data patterns created explicit exposure**, even when anonymized. Clients could be compelled to act on named supply chain risks, creating legal vulnerability. What they needed was guidance without the liability trail.

In response, I explored a solution that preserved client autonomy while delivering strategic value—leading to **a retrieval-augmented generation (RAG) AI concept**. Rather than surfacing deterministic data patterns that clients would need to interpret and act upon, this approach used **non-deterministic, conversational interactions** where clients could explore scenarios and receive contextual guidance drawn from their own data and Oritain's aggregated insights.

Through a private, conversational interface, clients could simulate scenarios and receive custom guidance on testing strategies—each assigned a unique ID as a guardrail against the non-deterministic nature of AI. They could then book those tests with a single click—getting closer to the most pressing need uncovered in research:

> Help me decide what to do next—without increasing my exposure.

The shift from deterministic data presentation to non-deterministic AI guidance would fundamentally change the liability equation: rather than creating an audit trail of aggregate patterns to interpret and act upon, clients would gain actionable, strategic insight through contextual guidance—reducing their exposure while preserving decision-making autonomy. While still exploratory, this direction would open the door to a **premium, AI-enabled offering**—balancing adaptability, insight, and client protection.
